...
| `tiles/momastela_ghirigori_0..2.jpg` | Momastela Ghirigori N — Bath A default floor | 31×62 cm |
| `tiles/keros_pisa_gris.jpg`        | Keros Pisa Gris — Bath B default floor | 33×33 cm |
| `tiles/alaplana_pune_blanco.jpg`   | Alaplana Pune Blanco (Carrara) — Bath B wall field | 30×60 cm |
| `tiles/alaplana_pune_cuadros.jpg`  | Alaplana Pune Cuadros mosaic décor | 30×60 cm |

**Tile catalog — type fixes texture AND dimensions.** Products live in `FLOOR_TILES` and `WALL_TILES`; each entry carries its label, its dimensions, and which texture to use (`tex` key for a single texture, or `mixed:true` for the Momastela face pack). The GUI exposes a **Tile type** dropdown per surface (`floor.prod`, `walls.<C>.prod`) — there are no free width/height sliders; the laid size is read from the chosen product (walls swap long/short by the per-wall `orient` toggle). Floor products: `0` Momastela 31×62 (mixed), `1` Kalina Gris 33×33, `2` Keros Pisa Gris 33×33. Wall field products: `0` Kalina Gris 25×50, `1` Alaplana Pune Blanco 30×60. Décor/accent products (`DECOR_TILE_LABEL`, used by `decorMat` + the band/accent selects): `0` Kalina patchwork, `1` Kalina Olas, `2` plain, `3` Alaplana Pune Cuadros. Add a product = push one catalog entry (+ its texture into `photoMat`/`colorMat`). **Defaults:** Bath A = Kalina walls + Momastela floor at 45°, front (S) wall a patchwork accent, the other three walls a 2-row Olas band; Bath B = Pune Blanco walls (stacked) + Pisa Gris floor, every wall a 2-row Pune Cuadros band, no accent; height 2.45, notch 0.20×1.20 back-left.
...
- **Room shape:** `roomPolygon(R)` returns a CCW point list (4 corners, or 6 for the notch). The three notch points must be inserted in CCW order — the point on the *incoming* edge first — which differs by corner parity (`[a,b,c2]` for even corners 0/2, `[c2,b,a]` for odd corners 1/3); getting this wrong yields a diagonal wall. `floorRects(R)` returns 1–2 axis-aligned rectangles covering the floor (the notch splits the L into two rects).
...
- **Wall tiling** (`buildWall`): each polygon edge → a wall in local (u=along, v=up) space. Tiles are axis-aligned rects, clipped to the wall and **carved around the door** with `rectSubtract`. Each edge's outward normal is oriented with `pointInPoly` (a small step along the normal that lands *inside* the room means it pointed inward, so flip) — this is robust for **concave notch walls**, where a centroid test gives the wrong side (and tiles end up facing out of the bathroom). The edge is then classified to a cardinal (N/E/S/W) by that normal and uses that cardinal's wall config (notch sub-walls inherit from the nearest cardinal). The door goes on the longest **outer-perimeter** edge of the chosen cardinal (an edge whose constant coordinate is on the bounding box), so a deep notch's inner wall never steals the door.
